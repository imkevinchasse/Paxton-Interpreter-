...
PROTECTED_FILES=("db.json" "training_data.json" "settings.json" "optimized_context.json" "audio_bank.json" "audio_bank" "whisper" "models" "audio" "uploads" ".env")
...
git clean -fd -e .backup -e db.json -e training_data.json -e settings.json -e optimized_context.json -e audio_bank.json -e audio_bank -e whisper -e models -e audio -e uploads -e .env
